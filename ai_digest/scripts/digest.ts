...
function fallbackSummaryText(article: Pick<Article, 'title' | 'description'>): string {
...
function buildScoringPrompt(articles: Array<{ index: number; title: string; description: string; sourceName: string }>): string {
  ...
  return `你是一位技術內容策展人，正在為一份面向技術愛好者的每日精選摘要篩選文章。

請對以下文章進行三個維度的評分（1-10 整數，10 分最高），並為每篇文章分配一個分類標籤和提取 2-4 個關鍵詞。

## 評分維度

### 1. 相關性 (relevance) - 對技術/程式/AI/網際網路從業者的價值
- 10: 所有技術人都應該知道的重大事件/突破
- 7-9: 對大部分技術從業者有價值
- 4-6: 對特定技術領域有價值
- 1-3: 與技術行業關聯不大

### 2. 質量 (quality) - 文章本身的深度和寫作品質
- 10: 深度分析，原創洞見，引用豐富
- 7-9: 有深度，觀點獨到
- 4-6: 資訊準確，表達清晰
- 1-3: 淺嚐即止或純轉述
...
- security: 資安、隱私、漏洞、加密相關
- engineering: 軟體工程、架構、程式語言、系統設計
- tools: 開發工具、開源專案、新發布的函式庫/框架
- opinion: 行業觀點、個人思考、職涯發展、文化評論
- other: 以上都不太適合的
...
function buildSummaryPrompt(
  ...
  return `你是一位技術內容摘要專家。請為以下文章完成三件事：

1. **繁體中文標題** (titleZh): 將英文標題翻譯成自然的繁體中文。如果原標題已經是中文則保持不變並轉換為繁體中文。
      ...
      try {
        ...
        for (const item of batch) {
          summaries.set(item.index, {
            titleZh: item.title,
            summary: fallbackSummaryText(item),
            reason: 'AI 摘要服務暫時不可用，先保留 RSS 原始摘要供快速判斷。',
          });
        }
      }
    });
...
直接返回纯文本总结，不要 JSON，不要 markdown 格式。`;
  ...
  try {
    ...
    return generateFallbackHighlights(articles);
  }
}
...
function generateFallbackHighlights(articles: ScoredArticle[]): string {
  ...
  const topCategories = Array.from(categoryCounts.entries())
    .sort((a, b) => b[1] - a[1])
    .slice(0, 3)
    .map(([category, count]) => `${CATEGORY_META[category].label} ${count} 篇`);
  ...
  const topTitles = articles.slice(0, 3).map(a => `「${a.titleZh || a.title}」`);

  const categoryText = topCategories.length > 0
    ? `今日精選主要集中在 ${topCategories.join('、')}。`
    : `今日精選涵蓋多個技術主題。`;
  const keywordText = topKeywords.length > 0
    ? `高頻話題包括 ${topKeywords.join('、')}，顯示今天的技術討論焦點偏向這些方向。`
    : '';
  const titleText = topTitles.length > 0
    ? `必讀文章以 ${topTitles.join('、')} 為代表，適合先從 Top 3 快速掌握重點。`
    : '';

  return [categoryText, keywordText, titleText].filter(Boolean).join('');
}
...
function generateDigestReport(articles: ScoredArticle[], highlights: string, stats: {
  ...
  // ── Footer ──
  report += `*生成於 ${dateStr} ${now.toISOString().split('T')[1]?.slice(0, 5) || ''} | 掃描 ${stats.successFeeds} 源 → 獲取 ${stats.totalArticles} 篇 → 精選 ${articles.length} 篇*\n`;
...
Environment:
  LLM_API_URL     LM Studio API URL (default: http://localhost:1234/v1/chat/completions)
...
async function main(): Promise<void> {
  ...
  if (!apiKey) {
    console.error('[digest] Error: No Gemini API Key found. Set GEMINI_API_KEY or GEMINI_API_KEYS env var.');
    console.error('[digest] Get one at: https://aistudio.google.com/apikey');
    process.exit(1);
  }
  console.log(`[digest] Using Gemini API (model: ${GEMINI_MODEL})`);
