...
### 全域輸出設定（強制）
- 預設輸出語言：**中文（繁體）**
- 除非你在任務中明確要求英文，否則一律以中文完成全文。
- 禁止先寫英文再翻譯；應直接以中文思考與寫作。
...
**數據出場節奏（強制）**：
- 開頭前 120 字內，至少 1 個硬數字（%、倍數、金額、時間）。
- 每 2~3 段至少 1 個硬數字，不可連續 4 段都沒數字。
- 每篇至少 5 個可記憶數字，其中至少 2 個要是對比數字（例如 `55.4% → 38.4%`）。
...
- 是否在開場 120 字內出現至少 1 個硬數字？
- 是否至少提供 2 組 `A → B` 對比？
- 是否至少 1 次明確寫出「絕對差（pp）」或「相對變化（%）」？
- 是否至少 1 次指出反直覺結果（例如高載入率但低成功率）？
- 若刪掉所有形容詞，文章還能靠數據說服嗎？
...
5. 若原始資料數據不足：先明確標註「可用數據不足」，再避免過度推論
6. 若原始素材含圖片：正文需嵌入對應圖片並加一行圖說，不可只放純文字連結

---

## 七、固定輸出與資料存儲規範（強制）

每次生成文章時，必須固定寫入以下路徑，並建立一個新資料夾存放本次內容：

`/Users/yankesswang/Documents/arthurwang_DB/Arthur_Blog/Posts`

### 7.1 資料夾建立規則
- 每次任務都建立新資料夾，不覆蓋既有內容。
- 資料夾命名格式：`YYYY-MM-DD_文章標題-slug`
- 若當日同名，尾碼加序號：`YYYY-MM-DD_文章標題-slug_02`

### 7.2 檔案存放規則
- 主文檔固定命名：`post_zh.md`
- 若有參考資料：`sources.md`
- 若有摘要版本：`summary_zh.md`
- 若原始資料含圖片：建立 `assets/` 放圖檔（建議 `figure-01.png` 命名）

### 7.4 圖片處理規範（來源有圖時強制）
- 原始資料出現圖表/截圖時，文章必須在對應段落插入圖片。
- 每張圖下方必須有一行圖說：`圖說：這張圖要讀者看懂什麼。`
- 優先插入最關鍵 3~6 張，避免無意義堆圖。
- 圖片插入格式：`![圖說](./assets/figure-01.png)`。
- 若原始資料提到圖片但檔案缺失，需在文中標註：`（原始圖檔缺失，待補）`。

### 7.3 寫作輸出行為
- 預設直接產出中文稿件到新資料夾內，不需再次詢問路徑。
- 只有在你明確指定其他路徑時，才可覆蓋此預設。
